...
public abstract class TQuad {
    // NOTE: value matches compile.pipeline.fluid.DefaultFluidRenderer.EPSILON (0.001f);
    // replaced here until the fluid pipeline batch is ported (see DefaultFluidRenderer TODO).
    private static final float TQUAD_EPSILON = 0.001f;
    /**
    ...
    static {
        ...
        // fraction
        float targetEpsilon = TQUAD_EPSILON * 2.1f;
    ...
    int initExtentsAndCenter(ChunkVertexEncoder.Vertex[] vertices) {
        ...
        if ((!this.facing.isAligned() || uniqueVertexes != 4) && uniqueVertexes >= 3) {
            float invUniqueVertexes = 1.0f / uniqueVertexes;
