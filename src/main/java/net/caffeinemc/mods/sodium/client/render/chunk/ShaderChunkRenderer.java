...
public abstract class ShaderChunkRenderer implements ChunkRenderer {
    ...
    private GlProgram<ChunkShaderInterface> createShader(String path, ChunkShaderOptions options) {
        ...
        GlShader vertShader = ShaderLoader.loadShader(ShaderType.VERTEX,
                new ResourceLocation("radium", path + ".vsh"), constants);

        GlShader fragShader = ShaderLoader.loadShader(ShaderType.FRAGMENT,
                new ResourceLocation("radium", path + ".fsh"), constants);

        try {
            return GlProgram.builder(new ResourceLocation("radium", "chunk_shader"))
    ...
    protected void begin(TerrainRenderPass pass) {
        ...
        gg.sona.radium.diag.Diag.once("shader-begin", "chunk shader bound: program=" + this.activeProgram.handle()
                + " fog=" + options.fog() + " pass=" + pass
                + " vertexFormatStride=" + this.vertexFormat.getStride()
                + " maxLodBias=" + RenderDevice.INSTANCE.getMaxTextureLodBias());
        gg.sona.radium.diag.Diag.glProbe("afterShaderSetup");
    }
