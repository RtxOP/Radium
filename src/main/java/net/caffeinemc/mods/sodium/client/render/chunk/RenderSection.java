...
public int getAdjacentMask() {
     ...
     */
    public void delete() {
        for (ChunkJob job : this.runningJobs) {
            job.setCancelled();
        }
    ...
    private int setRenderState(@NotNull BuiltSectionInfo info) {
        int prevFlags = this.region.getSectionFlags(this.sectionIndex);
    ...
    public boolean intersectSlopes(SectionPos origin, RenderSection other, int token) {
        int dx = Math.abs(origin.getX() - this.getChunkX());
        int dy = Math.abs(origin.getY() - this.getChunkY());
        int dz = Math.abs(origin.getZ() - this.getChunkZ());
    ...
    public boolean addBuildOutput(BuilderTaskOutput output) {
        boolean hasNoPendingOutputs = this.pendingBuildOutput == null && this.pendingDynamicSortOutput == null;

        if (output instanceof ChunkBuildOutput) {
            ChunkBuildOutput buildOutput = (ChunkBuildOutput) output;
            this.addMeshBuildOutput(buildOutput);
        } else if (output instanceof ChunkSortOutput) {
            ChunkSortOutput sortOutput = (ChunkSortOutput) output;
