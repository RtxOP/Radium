...
public class LevelColorCache {
    ...
    public int getColor(BiomeColors.ColorProvider resolver, int blockX, int blockY, int blockZ) {
        ...
        Slice[] slice = this.slices.get(resolver);

        if (slice[blockY].lastPopulateStamp < this.populateStamp) {
            this.updateColorBuffers(blockY, resolver, slice[blockY]);
        }

        ColorBuffer buffer = slice[blockY].getBuffer();
