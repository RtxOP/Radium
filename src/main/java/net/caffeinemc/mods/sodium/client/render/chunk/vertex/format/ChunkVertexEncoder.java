...
/**
 * Encodes vertex data for chunk meshes.
 *
 * <p>Port divergence from the reference: the LWJGL2 port has no {@code org.lwjgl.system.MemoryUtil},
 * so the encoder writes into a {@link ByteBuffer} at an explicit byte offset instead of taking a raw
 * pointer. The return value is the offset immediately after the written data.</p>
 */
public interface ChunkVertexEncoder {
    int write(ByteBuffer buffer, int offset, int materialBits, Vertex[] vertices, int sectionIndex);
