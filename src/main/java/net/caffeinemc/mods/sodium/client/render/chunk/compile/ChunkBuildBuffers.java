...
public class ChunkBuildBuffers {
    ...
    public BuiltSectionMeshParts createMesh(TerrainRenderPass pass, int visibleSlices, boolean forceUnassigned, boolean sliceReordering) {
        ...
        NativeBuffer mergedBuffer = new NativeBuffer(vertexTotal * this.vertexType.getVertexFormat().getStride());
        ...
        if (sliceReordering) {
            ...
            ChunkMeshBufferBuilder unassignedBuffer = builder.getVertexBuffer(ModelQuadFacing.UNASSIGNED);
            ...
            for (ModelQuadFacing facing : ModelQuadFacing.VALUES) {
                ...
                if (!buffer.isEmpty()) {
                    if (!forceUnassigned) {
                        int facingIndex = facing.ordinal();
                        int segmentIndex = facingIndex << 1;
        ...
        gg.sona.radium.diag.Diag.meshDump(mergedBuffer.getDirectBuffer(), this.vertexType.getVertexFormat().getStride(), vertexTotal, builder);
    ...
    public BuiltSectionMeshParts createModifiedTranslucentMesh(UpdatedQuadsList updatedQuads) {
        ...
        int stride = this.vertexType.getVertexFormat().getStride();
        int vertexTotal = TranslucentData.quadCountToVertexCount(updatedQuads.getMeshQuadCount());
        NativeBuffer mergedBuffer = new NativeBuffer(vertexTotal * stride);
        ByteBuffer mergedBufferBuilder = mergedBuffer.getDirectBuffer();
