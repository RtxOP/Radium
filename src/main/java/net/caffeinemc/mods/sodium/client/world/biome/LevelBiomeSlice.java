...
public class LevelBiomeSlice {
    ...
    private final BiomeGenBase[] biomes = new BiomeGenBase[SIZE * SIZE * SIZE];
    ...
    public void update(WorldClient level, ChunkRenderContext context) {
        ...
        for (int sectionX = 0; sectionX < 3; sectionX++) {
            for (int sectionY = 0; sectionY < 3; sectionY++) {
                for (int sectionZ = 0; sectionZ < 3; sectionZ++) {
                    this.copySectionBiomeData(context, sectionX, sectionY, sectionZ, BiomeGenBase.plains);
                }
            }
        }
    }

    private void copySectionBiomeData(ChunkRenderContext context, int sectionX, int sectionY, int sectionZ, BiomeGenBase defaultBiome) {
        ClonedChunkSection section = context.sections()[LevelSlice.getLocalSectionIndex(sectionX, sectionY, sectionZ)];
        BiomeGenBase[] biomeData = section.getBiomeData();
    ...
    private boolean hasUniformNeighbors(int cellX, int cellY, int cellZ) {
        BiomeGenBase biome = this.biomes[dataArrayIndex(cellX, cellY, cellZ)];
    ...
    public BiomeGenBase getBiome(int blockX, int blockY, int blockZ) {
    ...
    private BiomeGenBase getBiomeUsingVoronoi(int blockX, int blockY, int blockZ) {
        int x = blockX - 2;
        int y = blockY - 2;
        int z = blockZ - 2;
