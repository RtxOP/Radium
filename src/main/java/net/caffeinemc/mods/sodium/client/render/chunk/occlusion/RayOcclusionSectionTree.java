...
public class RayOcclusionSectionTree extends SectionTree implements OcclusionCuller.VisibilityTestingVisitor {
    ...
    public RayOcclusionSectionTree(Viewport viewport, float buildDistance, int frame, CullType cullType, World level) {
        ...
        this.minSection = 0;
        this.maxSection = SectionPos.blockToSectionCoord(level.getActualHeight());
    }
    ...
    private boolean isRayBlockedStepped(RenderSection section) {
        ...
        float x = (float) section.getCenterX();
        float y = (float) section.getCenterY();
        float z = (float) section.getCenterZ();
        float dX = (float) (this.transform.x - x);
        float dY = (float) (this.transform.y - y);
        float dZ = (float) (this.transform.z - z);
        ...
        int steps = Math.min((int) (length * RAY_MIN_STEP_SIZE_INV), RAY_TEST_MAX_STEPS);

        // avoid the last step being in the camera
        float stepsInv = 1.0f / steps;
        dX *= stepsInv;
        dY *= stepsInv;
        dZ *= stepsInv;
        ...
        for (int i = 1; i < steps; i++) {
            ...
            int result = this.blockHasObstruction((int) x, (int) y, (int) z);
            if (result == Tree.NOT_PRESENT) {
                // also test radius around to avoid false negatives
                float radius = SECTION_HALF_DIAGONAL * (steps - i) * stepsInv;

                // this pattern simulates a shape similar to the sweep of the section towards the camera
                boolean hasPath = false;
                for (int corner = 0; corner < 8; corner++) {
                    float offsetX = ((corner & 1) == 0) ? -radius : radius;
                    float offsetY = ((corner & 2) == 0) ? -radius : radius;
                    float offsetZ = ((corner & 4) == 0) ? -radius : radius;
    ...
    protected static class FlatTree extends Tree {
        ...
        public int getPresence(int x, int y, int z) {
            ...
            int bitIndex = interleave6x3(x, y, z);
            long mask = 1L << (bitIndex & 0b111111);
