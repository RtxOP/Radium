...
public class GLRenderDevice implements RenderDevice {
    ...
    public int getMaxTextureLodBias() {
        // The GL_MAX_TEXTURE_LOD_BIAS constant lives in GL14 in LWJGL2, but the
        // query method is inherited from GL11 (the GLxx classes do not subclass
        // each other in LWJGL2).
        return GL11.glGetInteger(GL14.GL_MAX_TEXTURE_LOD_BIAS);
    }
    ...
    private class ImmediateCommandList implements CommandList {
        ...
        @Override
        public void uploadDataToOffset(GlMutableBuffer glBuffer, int offset, ByteBuffer data, int size) {
            this.bindBuffer(GlBufferTarget.ARRAY_BUFFER, glBuffer);
            ...
            GL15.glBufferSubData(GlBufferTarget.ARRAY_BUFFER.getTargetParameter(), offset, slice);
        }
    ...
    private class ImmediateDrawCommandList implements DrawCommandList {
        ...
        public void multiDrawElementsBaseVertex(MultiDrawBatch batch, GlIndexType indexType) {
            ...
            // LWJGL2 has no glMultiDrawElementsBaseVertex; issue one draw call per entry.
            // The elementOffsets buffer stores byte offsets into the element buffer.
