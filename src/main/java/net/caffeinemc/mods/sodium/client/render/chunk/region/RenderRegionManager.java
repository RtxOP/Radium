...
public class RenderRegionManager {
    ...
    public void uploadResults(CommandList commandList, Collection<BuilderTaskOutput> results) {
        for (java.util.Map.Entry<RenderRegion, List<BuilderTaskOutput>> entry : this.createMeshUploadQueues(results)) {
    ...
    private void uploadResults(CommandList commandList, RenderRegion region, Collection<BuilderTaskOutput> results) {
        ...
        for (BuilderTaskOutput result : results) {
            ...
            if (result instanceof ChunkSortOutput) {
                ChunkSortOutput indexDataOutput = (ChunkSortOutput) result;
                if (!indexDataOutput.containsNewIndexData()) {
                ...
                if (sorter instanceof SharedIndexSorter) {
                    ...
                    NativeBuffer buffer = sorter.getIndexBuffer();
                    if (buffer == null) {
                        continue;
                    }
        ...
        boolean needsSharedIndexUpdate = translucentStorage != null && translucentStorage.needsSharedIndexUpdate();
        if (uploads.isEmpty() && indexUploads.isEmpty() && !needsSharedIndexUpdate) {
            return;
        }

        double[] cameraPosition = CameraUtils.getCameraPosition();
        ...
        profiler.startSection("upload_vertices");
        ...
        if (!uploads.isEmpty()) {
            ...
            for (PendingSectionMeshUpload upload : uploads) {
                ...
                if (upload.relativeBuiltTime != -1) { // We don't want the animation to happen again on chunks changing!
                    double dx = upload.section.getCenterX() - cameraPosition[0];
                    double dy = upload.section.getCenterY() - cameraPosition[1];
                    double dz = upload.section.getCenterZ() - cameraPosition[2];
        ...
        profiler.endSection();
        profiler.startSection("upload_indices");
    ...
    private Reference2ReferenceMap.FastEntrySet<RenderRegion, List<BuilderTaskOutput>> createMeshUploadQueues(Collection<BuilderTaskOutput> results) {
        Reference2ReferenceOpenHashMap<RenderRegion, List<BuilderTaskOutput>> map = new Reference2ReferenceOpenHashMap<RenderRegion, List<BuilderTaskOutput>>();

        for (BuilderTaskOutput result : results) {
            List<BuilderTaskOutput> queue = map.computeIfAbsent(result.section.getRegion(), k -> new ArrayList<>());
            queue.add(result);
        }
    ...
    private static class PendingSectionMeshUpload {
    public final RenderSection section;
    public final int relativeBuiltTime;
    public final BuiltSectionMeshParts meshData;
    public final TerrainRenderPass pass;
    public final PendingUpload vertexUpload;
    public PendingSectionMeshUpload(RenderSection section, int relativeBuiltTime, BuiltSectionMeshParts meshData, TerrainRenderPass pass, PendingUpload vertexUpload) {
        this.section = section;
        this.relativeBuiltTime = relativeBuiltTime;
    ...
    private static class PendingSectionIndexBufferUpload {
    public final RenderSection section;
    public final PendingUpload indexBufferUpload;
    public PendingSectionIndexBufferUpload(RenderSection section, PendingUpload indexBufferUpload) {
        this.section = section;
        this.indexBufferUpload = indexBufferUpload;
        }
