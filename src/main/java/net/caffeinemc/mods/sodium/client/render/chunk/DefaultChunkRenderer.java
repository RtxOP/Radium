...
public class DefaultChunkRenderer extends ShaderChunkRenderer {
    ...
    public void render(ChunkRenderMatrices matrices,
        ...
        gg.sona.radium.diag.Diag.glProbe("afterChunkDraw");
    }
    ...
    private static void addLocalIndexedDrawCommands(MultiDrawBatch batch, long pMeshData, int mask) {
        final LongBuffer pElementPointer = batch.elementOffsets;
        final IntBuffer pBaseVertex = batch.baseVertices;
        final IntBuffer pElementCount = batch.elementCounts;
        ...
        for (int facing = 0; facing < ModelQuadFacing.COUNT; facing++) {
            ...
            pElementCount.put(size, UInt32.uncheckedDowncast(elementCount));
            pBaseVertex.put(size, UInt32.uncheckedDowncast(baseVertex));

            // * 4 to convert to bytes (the index buffer contains integers)
            pElementPointer.put(size, elementOffset << 2);
    ...
    private static void addSharedIndexedDrawCommands(MultiDrawBatch batch, long pMeshData, int mask) {
        ...
        final long elementOffsetBytes = SectionRenderDataUnsafe.getBaseElement(pMeshData) << 2;
        ...
        for (int i = 0; i <= ModelQuadFacing.COUNT; i++) {
            ...
            if (maskBit == 0) {
                if (lastMaskBit == 1) {
                    ...
                    pElementCount.put(size, UInt32.uncheckedDowncast((groupVertexCount >> 2) * 6));
                    pBaseVertex.put(size, UInt32.uncheckedDowncast(baseVertex));
                    pElementPointer.put(size, elementOffsetBytes);
                    size++;
