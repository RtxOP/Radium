...
/**
 * MCP 1.8.9 port: the reference wraps modern MC's {@code CullingCameraView}; this wraps the
 * 1.8.9 {@link ICamera} (the vanilla {@code Frustum} instance mixed in via
 * {@code MixinBaseFrustum} to expose {@link ExtendedFrustum}).
 */
public final class SimpleFrustum implements Frustum {
    ...
    public SimpleFrustum(ICamera frustum) {
    ...
    private static String fmt(float[] p) {
        return String.format("[%.2f,%.2f,%.2f,%.2f]", p[0], p[1], p[2], p[3]);
    }
