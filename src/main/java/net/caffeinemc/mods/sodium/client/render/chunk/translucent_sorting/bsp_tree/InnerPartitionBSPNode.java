...
abstract class InnerPartitionBSPNode extends BSPNode {
     ...
     */
    static class NodeReuseData {
    private final int quadHash;
    private final int[] indexes;
    private final int indexCount;
    private final int maxIndex;
    public NodeReuseData(int quadHash, int[] indexes, int indexCount, int maxIndex) {
        this.quadHash = quadHash;
    ...
    static NodeReuseData prepareNodeReuse(BSPWorkspace workspace, IntArrayList indexes, int depth) {
        ...
        if (workspace.prepareNodeReuse && depth == 1 && indexes.size() > NODE_REUSE_THRESHOLD) {
            ...
            for (int i = 0; i < indexes.size(); i++) {
                ...
                TQuad quad = workspace.get(index);
    ...
    static BSPNode build(BSPWorkspace workspace, IntArrayList indexes, int depth, BSPNode oldNode) {
        ...
        if (oldNode instanceof InnerPartitionBSPNode) {
            InnerPartitionBSPNode oldInnerNode = (InnerPartitionBSPNode) oldNode;
            InnerPartitionBSPNode reusedNode = InnerPartitionBSPNode.attemptNodeReuse(workspace, indexes, oldInnerNode);
            if (reusedNode != null) {
                return reusedNode;
            }
        }
        ...
        for (int axisCount = 0; axisCount < 3; axisCount++) {
            ...
            for (int i = 0, size = indexes.size(); i < size; i++) {
                ...
                TQuad quad = workspace.get(quadIndex);
                float[] extents = quad.getExtents();
                ...
                switch (decodeType(point)) {
                    ...
                    case INTERVAL_SIDE: {
                                                ...
                                                if (thickness == 0) {
                                                    ...
                                                    if (canSplitQuads) {
                                                        float ownDistance = decodeDistance(point);
            ...
            if (partitions.size() <= 2) {
                ...
                Partition inside = partitions.get(0);
                Partition outside = partitions.size() == 2 ? partitions.get(1) : null;
        ...
        if (canSplitQuads) {
            ...
            BSPNode multiLeafNode = buildTopoMultiLeafNode(workspace, indexes, true);
            ...
            BSPNode multiLeafNode = buildTopoMultiLeafNode(workspace, indexes, false);
            if (multiLeafNode == null) {
    ...
    static void flushBestSplittingGroup(IntArrayList splittingGroup, IntArrayList bestSplittingGroup, int axis) {
        ...
        int newSize = bestSplittingGroup.size();
    ...
    static private void splitQuadEven(int vertexInsideMap, FullTQuad insideQuad, FullTQuad outsideQuad, Vector3fc splitPlane, float splitDistance) {
        ...
        for (int indexA = 0; indexA < 4; indexA++) {
            int indexB = (indexA + 1) & 0b11;
            boolean insideA = (vertexInsideMap & (1 << indexA)) != 0;
            boolean insideB = (vertexInsideMap & (1 << indexB)) != 0;
            if (insideA == insideB) {
                continue;
            }
    ...
    static private void splitQuadOdd(int cornerIndex, FullTQuad cornerQuad, FullTQuad cutQuad, FullTQuad bulkQuad, Vector3fc splitPlane, float splitDistance) {
        ...
        ChunkVertexEncoder.Vertex[] cornerVertices = cornerQuad.getVertices(); // corner split off by the plane
        ChunkVertexEncoder.Vertex[] cutVertices = cutQuad.getVertices(); // quad between the corner and the bulk
        ChunkVertexEncoder.Vertex[] bulkVertices = bulkQuad.getVertices(); // quad that retains the three non-cut vertices

        int prevIndex = (cornerIndex - 1) & 0b11;
        int nextIndex = (cornerIndex + 1) & 0b11;
        int oppositeIndex = (cornerIndex + 2) & 0b11;
    ...
    static private void splitTriangleCorner(int cornerIndex, FullTQuad cornerQuad, FullTQuad bulkQuad, Vector3fc splitPlane, float splitDistance) {
        ...
        ChunkVertexEncoder.Vertex[] cornerVertices = cornerQuad.getVertices(); // corner split off by the plane
        ChunkVertexEncoder.Vertex[] bulkVertices = bulkQuad.getVertices(); // quad that retains the other vertices
    ...
    private static void interpolateAttributes(float splitDistance, Vector3fc splitPlane, ChunkVertexEncoder.Vertex inside, ChunkVertexEncoder.Vertex outside, ChunkVertexEncoder.Vertex targetA, ChunkVertexEncoder.Vertex targetB, ChunkVertexEncoder.Vertex targetC) {
        ...
        float newAo = Mth.lerp(outsideAmount, inside.ao, outside.ao);
        float newU = Mth.lerp(outsideAmount, inside.u, outside.u);
        float newV = Mth.lerp(outsideAmount, inside.v, outside.v);

        float newLightBl = Mth.lerp(outsideAmount, inside.light & 0xFF, outside.light & 0xFF);
        float newLightSl = Mth.lerp(outsideAmount, inside.light >> 16, outside.light >> 16);
    ...
    static private BSPNode handleIntersecting(BSPWorkspace workspace, IntArrayList indexes, int depth, BSPNode oldNode) {
        ...
        int primaryIntersectorThreshold = MathHelper.clamp_int(indexes.size() / 2, 2, 4);
        ...
        while (true) {
            ...
            TQuad quadA = workspace.get(indexes.getInt(i));
            TQuad quadB = workspace.get(indexes.getInt(j));
