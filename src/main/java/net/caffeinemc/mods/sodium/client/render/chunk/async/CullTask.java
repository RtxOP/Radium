...
public class CullTask extends AsyncRenderTask<CullResult> {
    ...
    protected CullResult runTask() {
        TaskCollectingTree wideTree = new TaskCollectingTree(this.viewport, this.searchDistanceRegular, this.frame, CullType.WIDE, this.level);
        SectionTree regularTree = new SectionTree(this.viewport, this.searchDistanceRegular, this.frame, CullType.REGULAR, this.level);
        RayOcclusionSectionTree localTree = new RayOcclusionSectionTree(this.viewport, this.searchDistanceLocal, this.frame, CullType.LOCAL, this.level);
    ...
    protected void applyPresentPatches(SectionTree result) {
        ...
        for (RenderSection section : this.presentPatches) {
