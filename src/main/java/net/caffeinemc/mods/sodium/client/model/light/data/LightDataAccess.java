...
public abstract class LightDataAccess {
    ...
    public int get(int x, int y, int z, EnumFacing d1, EnumFacing d2) {
    ...
    public int get(int x, int y, int z, EnumFacing dir) {
        return this.get(x + dir.getFrontOffsetX(),
                y + dir.getFrontOffsetY(),
                z + dir.getFrontOffsetZ());
    }

    public int get(BlockPos pos, EnumFacing dir) {
    ...
    protected int compute(int x, int y, int z) {
        ...
        boolean em = block.getLightValue() > 1;
        boolean op = block.getMaterial().isOpaque();
        boolean fo = block.isFullBlock() && block.getMaterial().isOpaque();
        boolean fc = block.isOpaqueCube();

        int lu = state.getBlock().getLightValue();
        ...
        if (fo && lu == 0) {
            ...
            bl = level.getLight(EnumSkyBlock.BLOCK, pos);
            sl = level.getLight(EnumSkyBlock.SKY, pos);
        }

        float ao = block.getAmbientOcclusionLightValue();
