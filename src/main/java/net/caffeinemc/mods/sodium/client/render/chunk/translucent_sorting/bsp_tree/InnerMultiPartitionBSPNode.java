...
class InnerMultiPartitionBSPNode extends InnerPartitionBSPNode {
    ...
    void addPartitionPlanes(BSPWorkspace workspace) {
        ...
        for (BSPNode partition : this.partitions) {
            if (partition instanceof InnerPartitionBSPNode) {
                ((InnerPartitionBSPNode) partition).addPartitionPlanes(workspace);
            }
        }
    }
    ...
    static BSPNode buildFromPartitions(BSPWorkspace workspace, IntArrayList indexes, int depth, BSPNode oldNode,
        ...
        if (oldNode instanceof InnerMultiPartitionBSPNode) {
            InnerMultiPartitionBSPNode multiNode = (InnerMultiPartitionBSPNode) oldNode;
            if (multiNode.axis == axis && multiNode.partitions.length > 0) {
