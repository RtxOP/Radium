...
public abstract class BlendedColorProvider implements ColorProvider {
    ...
    private int getVertexColor(LevelSlice slice, IBlockState state, BlockPos pos, ModelQuadView quad, int vertexIndex) {
        ...
        if (prov == null) {
            ...
            m00 = this.getColor(prov, slice, state, scratchPos.set(blockX + 0, blockY, blockZ + 0), quad.getTintIndex());
            m01 = this.getColor(prov, slice, state, scratchPos.set(blockX + 0, blockY, blockZ + 1), quad.getTintIndex());
            m10 = this.getColor(prov, slice, state, scratchPos.set(blockX + 1, blockY, blockZ + 0), quad.getTintIndex());
            m11 = this.getColor(prov, slice, state, scratchPos.set(blockX + 1, blockY, blockZ + 1), quad.getTintIndex());
        }
