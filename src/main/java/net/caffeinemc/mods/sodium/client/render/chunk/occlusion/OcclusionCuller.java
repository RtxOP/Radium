...
public class OcclusionCuller {
    ...
    private static long getAngleVisibilityMaskWide(Viewport viewport, RenderSection section, int width) {
        ...
        float dx = Math.abs(origin.minBlockX() + 8 - section.getCenterX());
        float dy = Math.abs(origin.minBlockY() + 8 - section.getCenterY());
        float dz = Math.abs(origin.minBlockZ() + 8 - section.getCenterZ());
    ...
    public void findVisible(GraphOcclusionVisitor visitorWide,
        ...
        WriteQueue<RenderSection> initWriteQueue = this.queue.write();
    ...
    private void processQueue(ReadQueue<RenderSection> readQueue,
        ...
        SectionPos origin = this.viewport.getChunkCoord();

        // only visible sections are entered into the queue
        while ((section = readQueue.dequeue()) != null) {
            int incomingDirectionsWide = section.getIncomingDirectionsWide();
            int incomingDirectionsRegular = section.getIncomingDirectionsRegular();
            int incomingDirectionsLocal = section.getIncomingDirectionsLocal();
            ...
            if (this.useOcclusionCulling) {
                long[] visibilityDataSet = section.getVisibilityData();
                ...
                long visibilityDataWide = this.joinVisibilityData(visibilityDataSet, section, this.viewport, 2);
                long visibilityDataRegular = this.joinVisibilityData(visibilityDataSet, section, this.viewport, 1);
                long visibilityDataLocal = this.joinVisibilityData(visibilityDataSet, section, this.viewport, 0);
    ...
    private void visitNeighbors(WriteQueue<RenderSection> queue, RenderSection section, int outgoingWide, int outgoingRegular, int outgoingLocal, SectionPos origin) {
        ...
        RenderSection originSection = origin == null ? null : section;
    ...
    private void tryVisitNode(WriteQueue<RenderSection> queue, RenderSection originSection, int outgoingWide, int outgoingRegular, int outgoingLocal, RenderSection section, int outgoingDirection) {
        ...
        boolean hasWidePath = GraphDirectionSet.contains(outgoingWide, outgoingDirection);
        boolean hasRegularPath = GraphDirectionSet.contains(outgoingRegular, outgoingDirection);
        boolean hasLocalPath = GraphDirectionSet.contains(outgoingLocal, outgoingDirection);
    ...
    private void visitNode(WriteQueue<RenderSection> queue, RenderSection section, int outgoingDirection, boolean hasLocalPath, boolean hasRegularPath, boolean hasWidePath) {
        ...
        int incomingSet = GraphDirectionSet.of(GraphDirection.opposite(outgoingDirection));
        if (hasWidePath) {
    ...
    private void init(WriteQueue<RenderSection> queue) {
        SectionPos origin = viewport.getChunkCoord();
        int maxHeight = SectionPos.blockToSectionCoord(level.getActualHeight());
    ...
    private void initWithinWorld(WriteQueue<RenderSection> queue) {
        RenderSection originSection = this.getRenderSection(this.origin.getX(), this.origin.getY(), this.origin.getZ());
        ...
        if (this.useOcclusionCulling) {
            ...
            long[] visibilityDataSet = originSection.getVisibilityData();
            if (visibilityDataSet == null) {
                // No visibility data, so we can't traverse into any neighbors.
                return;
            }

            long visibilityDataWide = this.joinVisibilityData(visibilityDataSet, originSection, this.viewport, 2);
            long visibilityDataRegular = this.joinVisibilityData(visibilityDataSet, originSection, this.viewport, 1);
            long visibilityDataLocal = this.joinVisibilityData(visibilityDataSet, originSection, this.viewport, 0);
    ...
    private boolean initOutsideWorldHeight(WriteQueue<RenderSection> queue) {
        int radius = MathHelper.floor_float(this.searchDistanceRegular / 16.0f);
