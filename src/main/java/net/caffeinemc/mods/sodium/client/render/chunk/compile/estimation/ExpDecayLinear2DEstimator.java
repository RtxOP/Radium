...
public abstract class ExpDecayLinear2DEstimator<C> extends Abstract2DLinearEstimator<
    ...
    protected ClearingLinearRegressionBatch<C> createNewDataBatch() {
        return new ClearingLinearRegressionBatch<C>();
    }
    ...
    protected static class ExpDecayLinearFunction<C> extends Abstract2DLinearEstimator.LinearFunction<
        ...
        public void update(ClearingLinearRegressionBatch<C> batch) {
            ...
            long xSum = 0;
            long ySum = 0;
            for (Abstract2DLinearEstimator.DataPair<C> data : batch) {
                xSum += data.x();
                ySum += data.y();
            }
            float xMean = (this.xMeanOld * oldDataWeight + xSum) * totalWeightInv;
            float yMean = (this.yMeanOld * oldDataWeight + ySum) * totalWeightInv;

            // the covariance and variance are calculated from the differences to the mean
            float covarianceSum = 0.0f;
            float varianceSum = 0.0f;
            for (Abstract2DLinearEstimator.DataPair<C> data : batch) {
                float xDelta = data.x() - xMean;
                float yDelta = data.y() - yMean;
    ...
    protected ExpDecayLinearFunction<C> createNewModel() {
        return new ExpDecayLinearFunction<C>(this.newDataRatio, this.initialSampleTarget, this.minBatchSize, this.initialOutput);
    }
}
