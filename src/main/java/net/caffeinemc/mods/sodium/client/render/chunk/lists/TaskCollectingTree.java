...
public class TaskCollectingTree extends SectionTree {
    ...
    protected void addPendingSection(RenderSection section, int type, boolean inFrustum) {
        ...
        int localX = section.getChunkX() - this.baseOffsetX;
        int localY = section.getChunkY() - SECTION_Y_MIN;
        int localZ = section.getChunkZ() - this.baseOffsetZ;
    ...
    private float getSectionPriority(RenderSection section, int type, boolean inFrustum) {
        ...
        // alternatively: float distance = deltaX + deltaY + deltaZ;
        float deltaX = Math.abs(section.getCenterX() - this.cameraX);
        float deltaY = Math.abs(section.getCenterY() - this.cameraY);
        float deltaZ = Math.abs(section.getCenterZ() - this.cameraZ);
        ...
        long taskPendingTimeNanos = this.creationTime - section.getPendingUpdateSince();
