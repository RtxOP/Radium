...
public class SmoothLightPipeline implements LightPipeline {
    ...
    @Override
    public void calculate(ModelQuadView quad, BlockPos pos, QuadLightData out, EnumFacing cullFace, EnumFacing lightFace, boolean shade, boolean enhanced) {
     ...
     */
    private void applyAlignedFullFace(AoNeighborInfo neighborInfo, BlockPos pos, EnumFacing dir, QuadLightData out, boolean shade) {
     ...
     * Flags: IS_ALIGNED, IS_PARTIAL
     */
    private void applyAlignedPartialFace(AoNeighborInfo neighborInfo, ModelQuadView quad, BlockPos pos, EnumFacing dir, QuadLightData out, boolean shade) {
        for (int i = 0; i < 4; i++) {
     ...
     */
    private void applyParallelFace(AoNeighborInfo neighborInfo, ModelQuadView quad, BlockPos pos, EnumFacing dir, QuadLightData out, boolean shade) {
        for (int i = 0; i < 4; i++) {
            // Clamp the vertex positions to the block's boundaries to prevent weird errors in lighting
            float cx = clamp(quad.getX(i));
            float cy = clamp(quad.getY(i));
     ...
     * Flags: !IS_ALIGNED, !IS_PARALLEL
     */
    private void applyNonParallelFace(AoNeighborInfo neighborInfo, ModelQuadView quad, BlockPos pos, EnumFacing dir, QuadLightData out, boolean shade) {
        for (int i = 0; i < 4; i++) {
            // Clamp the vertex positions to the block's boundaries to prevent weird errors in lighting
            float cx = clamp(quad.getX(i));
            float cy = clamp(quad.getY(i));
    ...
    private void applyAlignedPartialFaceVertex(BlockPos pos, EnumFacing dir, float[] w, int i, QuadLightData out, boolean offset, boolean shade) {
    ...
    private void applyInsetPartialFaceVertex(BlockPos pos, EnumFacing dir, float n1d, float n2d, float[] w, int i, QuadLightData out, boolean shade) {
    ...
    private AoFaceData gatherInsetFace(ModelQuadView quad, BlockPos blockPos, int vertexIndex, EnumFacing lightFace, boolean shade) {
     ...
     */
    private void applyAmbientLighting(final float[] brightness, EnumFacing face, boolean shade) {
     ...
     */
    private float getAmbientBrightness(EnumFacing face, boolean shade) {
     ...
     */
    private AoFaceData getCachedFaceData(BlockPos pos, EnumFacing face, boolean offset, boolean shade) {
