...
/**
 * Internal use only.
 * <p>Port divergence from the reference: {@code String.format} is used instead of
 * {@code String.formatted} (Java 8 compatibility).</p>
 */
public class DependencyInjection {
    public static <T> T load(Class<T> apiClass, String implClassName) {
        ...
        if (!apiClass.isAssignableFrom(implClass)) {
            throw new RuntimeException(String.format("Class %s does not implement interface %s",
                    implClass.getName(), apiClass.getName()));
        }
