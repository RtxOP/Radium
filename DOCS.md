...
| **4. ModelView Matrix Translation Zeroed (Vertical Offset Bug)** | [`SodiumWorldRenderer.java:113-122`](file:///home/e/dev/SodiumForge/src/main/java/net/caffeinemc/mods/sodium/client/render/SodiumWorldRenderer.java#L113-L122) | `captureGlMatrices()` zeroed the `GL_MODELVIEW` translation. In 1.8.9 the modelview translation is *only* the eye-height offset (`orientCamera` ends with `translate(0, -eyeHeight, 0)` — the camera position is NOT in the modelview; it is applied per-chunk in vanilla). `u_RegionOffset` is feet-based, so `world - feet - eyeHeight = world - eye` requires that translation. Zeroing it rendered geometry at `world - feet` — 1.6 blocks above the eye (the "player sinks / surface above head" bug). Removed the zeroing; the matrix is now passed through unchanged, matching the reference's `Camera.MODEL_MATRIX` (`ActiveRenderInfo.MODELVIEW`, read after `setupCameraTransform`). |
...
Notes:
- ProGuard 6.2.2 runs on the Java 8 build JVM and reads `<java.home>/lib/rt.jar` as `-libraryjars` (JDK 8 bytecode, class-file major 52). **Preverification is enabled** (no `-dontpreverify`): shrinking removes members, and the preverifier is what rewrites the Java 8 `StackMapTable` correctly. With `-dontpreverify` the stale frames fail verification at runtime — `VerifyError: Expecting a stackmap frame` (observed in `Long2IntFunction.remove` during `ChunkTracker.<init>`). `rt.jar` on `-libraryjars` satisfies the preverifier's partial evaluation (without it, preverify itself fails).
...
- The output is validated locally: all 1 271 shrunk classes load with `-Xverify:all` on a Java 8 JVM (0 verification errors).
...
| **Camera ModelView Matrix** | [`net.caffeinemc.mods.sodium.client.render.SodiumWorldRenderer`](file:///home/e/dev/SodiumForge/src/main/java/net/caffeinemc/mods/sodium/client/render/SodiumWorldRenderer.java) | Removed the `setTranslation(0,0,0)` zeroing in `captureGlMatrices()`; the GL modelview is now passed through unchanged (matching the reference's `Camera.MODEL_MATRIX`). | Fixes the vertical 1.6-block offset (geometry above the eye). The modelview translation is the eye-height offset that combines with the feet-based `u_RegionOffset` to yield eye-relative coordinates; zeroing it broke Y. |
